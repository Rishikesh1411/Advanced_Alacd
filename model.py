...
from utils import AdaptiveAlpha, TempFromEntropy, jensen_shannon_divergence

class Base_Model:
    def __init__(self, model_name, device, num_gpus, max_gpu_memory=24, precision="bf16"):
        """
    ...
    def load_model(self, model_name):
        ...
        #
        # NEW CODE by Rishikesh Raj:
        # No 4-bit/8-bit quantization. Use full model weights with explicit
        # bf16/fp16/fp32 precision. bf16 is best on A100/H100/L4-class GPUs.
        precision_to_dtype = {
            "bf16": torch.bfloat16,
            "fp16": torch.float16,
            "fp32": torch.float32,
        ...
        dtype = precision_to_dtype.get(str(self.precision).lower(), torch.bfloat16 if self.device == "cuda" else torch.float32)
        if self.device == "cpu":
            dtype = torch.float32
        kwargs = {"torch_dtype": dtype, "low_cpu_mem_usage": True}
        ...
        # Prepare model kwargs - use selected non-quantized precision.
        model_kwargs = kwargs.copy()
        model_kwargs["device_map"] = "auto" if self.device == "cuda" else None
        if self.device == "cuda" and self.num_gpus != "auto":
            try:
                gpu_count = int(self.num_gpus)
                if gpu_count > 1:
                    model_kwargs["max_memory"] = {i: f"{self.max_gpu_memory}GiB" for i in range(gpu_count)}
            except Exception:
                pass
        ...
        self.device = "cuda" if torch.cuda.is_available() and str(self.num_gpus) != "0" else "cpu"
        if self.device == "cuda":
        ...
        #
        # NEW CODE by Rishikesh Raj:
        # Do not call .to(cuda) after device_map="auto"; Accelerate has already
        # placed the full-precision shards. For CPU, move normally.
        if self.device == "cpu":
            self.model = model.to(self.device)
        else:
            self.model = model
    ...
    def _generate_contrast_layer_adjusted_context_jsd(
        ...
        for _ in range(max_new_tokens):
            ...
            final_logits_context = self.adapter.get_logits_from_hidden_state(out_ctx.hidden_states[-1][:, -1:, :])
            final_logits_no_context = self.adapter.get_logits_from_hidden_state(out_nocxt.hidden_states[-1][:, -1:, :])
            ...
            for layer_idx, (ctx_hid, nocxt_hid) in enumerate(
                ...
                if layer_idx >= start_layer:
                    mid_logits_ctx = self.adapter.get_logits_from_hidden_state(ctx_hid[:, -1:, :])
                    mid_logits_nocxt = self.adapter.get_logits_from_hidden_state(nocxt_hid[:, -1:, :])
    ...
    def _generate_advanced_contrast_layer_adjusted_context_jsd(
        ...
        for _ in range(max_new_tokens):
            ...
            if len(jsd_divergences) == 0:
                ...
                #
                # NEW CODE:
                # Use the explicit AdaptiveAlpha(max_JSD, p_final, p_mid) formula
                # from utils.py. This makes Algorithm 1 reproducible and gates
                # high-JSD layers by confidence instead of assuming disagreement
                # automatically means truthfulness.
                effective_layer_alpha = AdaptiveAlpha(
                    best_jsd,
                    final_top_prob_val,
                    mid_top_prob_val,
                    alpha_min=max(0.25, min(float(layer_alpha), 0.65)),
                    alpha_max=0.82,
                )
                ...
                #
                # NEW CODE:
                # Use the explicit TempFromEntropy(H) formula from utils.py.
                temp = TempFromEntropy(prob_entropy.item(), t_min=0.70, t_max=0.80, entropy_scale=12.0)
                ...
                self.last_generation_trace.append({
                    "mode": "ALACD",
                    "selected_layer": int(top_layers[0][1]),
                    "top_layers": [int(item[1]) for item in top_layers],
                    "max_jsd": float(best_jsd),
                    "final_top_prob": float(final_top_prob_val),
                    "mid_top_prob": float(mid_top_prob_val),
                    "adaptive_alpha": float(effective_layer_alpha),
